# Cloud Pak Behavior Analysis Service gitops module

Module to provision a gitops repo with the resources necessary to provision a Cloud Pak Behavior Analysis Service instance on a cluster. In order to provision the instance, the following steps are performed:

1. Add the BAS operator chart to the gitops repo (charts/bas-operator)
2. Generate sealed secrets for the MongoDB credentials and the Grafana credentials
3. Add the BAS instance chart to the gitops repo (charts/bas-instance)

Currently the BAS instance helm chart produces an Analytics Proxy instance and generates an api key using the GenerateKey resource.
